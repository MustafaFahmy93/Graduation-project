...
def end2endTread():
    ...
    while (end_app==False):  
        if(endtoendFlag == True)  :
            ...
            print("end to end angle  :"+str(degrees))
...
def gpsThread():
    ...
    while (True):
        ...
        print("distance[i]=> " + path[i] + "(" + str(distance(current_position, coordinates[path[i]])) + ")")
        ...
        #     dc = 1
        #     endToEndFlag=True
        if ((distance(current_position, coordinates[path[i]]) < 3) and (i + 1 < len(path))):
            count_next+=1
            if(count_next==3):
                dc = 0.8 
                i += 1  # next state
                print("distance[i]=> " + path[i] + "(" + str(distance(current_position, coordinates[path[i-1]])) + ")")
                count_next=0
                endToEndFlag=False
                print("stop end 2 end")
