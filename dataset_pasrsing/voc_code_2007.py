...
for the_class in classes:
    for image_set in sets:
        ...
        image_ids = open(path_2007+'/ImageSets/Main/%s_%s.txt'%(the_class, image_set)).read().strip().split()
        ...
        for image_id ,truth_ground in zip (new_image_ids ,truth_grounds) :
            ...
            if truth_ground =='1' :
                print(image_id)
                src_l = path_2007 +'/annotations/'+image_id+'.xml'
                dst_l = path_2007 + '/labels/2007_'+ image_id+'.xml'
                src_img=path_2007 + '/JPEGImages/' + image_id+'.jpg'
                dst_img=path_2007 + '/my_images/2007_' + image_id + '.jpg'
                print(src_l, dst_l)
